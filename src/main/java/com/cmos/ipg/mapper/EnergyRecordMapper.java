...
public interface EnergyRecordMapper {
    @Select("SELECT * FROM ip_energy_record WHERE device_code=#{deviceCode} and park_code=#{parkCode} LIMIT 1")
    @Results(value ={
    ...
    EnergyRecord findByDeviceCode(@Param("deviceCode") String deviceCode ,@Param("parkCode") String parkCode);
    ...
    @Delete("DELETE  FROM ip_energy_record WHERE device_name = #{deviceName}and deviceCode = #{deviceCode}and park_code=#{parkCode}")
    void deleteByNameAndCode(@Param("deviceName")String deviceNamem,@Param("deviceCode")String deviceCode,@Param("parkCode") String parkCode);

    @Update("UPDATE ip_energy_record SET device_name = #{deviceName} WHERE device_code = #{deviceCode} and park_code=#{parkCode}")
    void update(@Param("deviceName") String deviceName,@Param("deviceCode") String deviceCode,@Param("parkCode") String parkCode);
}
