...
public interface DeviceStatusMapper {
    ...
    @Select("SELECT * FROM ip_device_status WHERE device_id = #{deviceId} and park_code=#{parkCode} LIMIT 1")
    @Results(value = {
    ...
    DeviceStatus findByDeviceId(@Param("deviceId") int deviceId ,@Param("parkCode") String parkCode);

    @Insert("INSERT INTO ip_device_status(device_id, device_name,device_code,device_location,device_para_name,device_para_value,park_code)" +
            "VALUES(#{deviceStatus.deviceId}, #{deviceStatus.deviceName},#{deviceStatus.deviceCode},#{deviceStatus.deviceLocation},#{deviceStatus.deviceParaName}, #{deviceStatus.deviceParaValue}, #{deviceStatus.parkCode})")
    void save(@Param("deviceStatus")DeviceStatus deviceStatus);

    @Delete("DELETE  FROM ip_device_status WHERE device_name = #{deviceName} and  device_para_name = #{deviceParaName} and park_code=#{parkCode}")
    void deleteByNameAndPara(@Param("deviceName") String deviceName,@Param("deviceParaName") String deviceParaName,@Param("parkCode") String parkCode);
}
