...
public interface EnergyRecordDetailMapper {
    @Select("SELECT * FROM ip_energy_record_detail WHERE device_id=#{deviceId} and park_code=#{parkCode} LIMIT 1")
    @Results(value = {
    ...
    EnergyRecordDetail findByDeviceCode(@Param("deviceId") int deviceId ,@Param("parkCode") String parkCode);
    ...
    @Delete("DELETE  FROM ip_energy_record_detail WHERE device_name = #{deviceName}and device_id = #{deviceId} and park_code=#{parkCode}")
    void deleteByNameAndCode(@Param("deviceName") String deviceNamem, @Param("deviceId") int deviceId,@Param("parkCode") String parkCode);

    @Update("UPDATE ip_energy_record_detail SET device_name = #{deviceName} WHERE device_id = #{deviceId} and park_code=#{parkCode}")
