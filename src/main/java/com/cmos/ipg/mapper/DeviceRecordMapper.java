...
public interface DeviceRecordMapper {
    ...
    @Select("SELECT * FROM ip_device_record WHERE device_code=#{deviceCode} and park_code=#{parkCode} LIMIT 1")
    @Results(value ={
    ...
    DeviceRecord findByDeviceCode(@Param("deviceCode") String deviceCode ,@Param("parkCode") String parkCode);
    ...
    @Delete("DELETE  FROM ip_device_record WHERE device_name = #{deviceName}and deviceCode = #{deviceCode} and park_code=#{parkCode}")
    void deleteByNameAndCode(@Param("deviceName")String deviceNamem,@Param("deviceCode")int deviceCode,@Param("parkCode") String parkCode);

    @Update("UPDATE ip_device_record SET device_name = #{deviceName} WHERE device_code = #{deviceCode}and park_code=#{parkCode}")
    void update(@Param("deviceName") String deviceName,@Param("deviceCode") int deviceCode,@Param("parkCode") String parkCode);
}
